...
const Card = ({ data, handleAddToCart }) => {
    ...
    const handleLikeToggle = async (item) => {
        ...
        try {
            ...
            if (isLiked) {
                await axios.post('https://farxodovich1811.pythonanywhere.com/api/unlike_item/', { id: item.id });
                setLikedItems(likedItems.filter(likedItem => likedItem.id !== item.id)); 
            } else {
                await axios.post('https://farxodovich1811.pythonanywhere.com/api/like_item/', { id: item.id });
