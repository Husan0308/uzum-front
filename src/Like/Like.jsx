...
export default function Like() {
    ...
    const handleAddToCart = async (item) => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/add-item/', { id: item.id });
    ...
    useEffect(() => {
        (async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/get_cart_items/');
    ...
    const removeFromCart = async (itemId) => {
        try {
            ...
            const response = await axios.delete(`http://127.0.0.1:8000/api/delete_from_liked/${itemId}/`);
                    ...
                    {liked.map(item => (
                        <div key={item.id} className="card">
                            <div className="card-image">
                                <a className="liked-icon" onClick={() => removeFromCart(item.id)}>
                                    <IoMdHeart />
                                </a>
                                <img 
